...
-(void)createTableView
{
    ...
    NSInteger userOrder = [[[NSUserDefaults standardUserDefaults] objectForKey:@"writeAllOrder"] integerValue];
    if (userOrder==0){
...
- (void)loadAllWrite
{
    ...
    [appDelegate.sessionManager GET:[NSString stringWithFormat:@"%@%@",__kBaseURLString,ptxGetUserTaskOrderByUserID] parameters:rankingDic success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        [[NSUserDefaults standardUserDefaults]setObject:order forKey:@"writeAllOrder"];
    } failure:^(NSURLSessionDataTask *task, NSError *error) {
