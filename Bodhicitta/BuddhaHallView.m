...
-(void)setModel:(TempleImage *)model
{
    ...
    [self.teacupImageView1 setImageWithURL:[NSURL URLWithString:model.teaUrl] placeholderImage:[UIImage imageNamed:@"茶杯"]];
...
#pragma mark --茶--
    NSArray  *teaCupArray = @[self.teacupImageView1,self.teacupImageView2,self.teacupImageView3];
    CGFloat teaCupMargin = 10;
    CGFloat teaCupWidth =  (width - 2*teaCupMargin) / 3;
    CGFloat teaCupHeight = teaCupWidth * 1.5;
    for(int i = 0 ; i < teaCupArray.count ; i++)
    {
        UIImageView *imageView  = teaCupArray[i];
        imageView.frame = CGRectMake(CGRectGetMaxX(self.fruitBowlImageView1.frame) + marginX+(teaCupMargin+teaCupWidth) * i,self.frame.size.height - marginV - teaCupHeight*3.2,teaCupWidth,teaCupHeight);
        imageView.userInteractionEnabled=YES;
        imageView.image = [UIImage imageNamed:@"茶杯"];
        UITapGestureRecognizer * teaTap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(teaTapClick)];
        [imageView addGestureRecognizer:teaTap];
