...
-(void)createBar
{
    ...
    self.titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/2-50, 0,SCREEN_WIDTH - 50 , _navBar.frame.size.height)];
...
-(void)PtxAddUserTaskRecord:(TributeModel *) model
{
    ...
    if (model.taskType == 2) {
        //敬茶
        [view.teacupImageView1 setImageWithURL:[NSURL URLWithString:model.materialImageUrl]];
        [view.teacupImageView2 setImageWithURL:[NSURL URLWithString:model.materialImageUrl]];
        [view.teacupImageView3 setImageWithURL:[NSURL URLWithString:model.materialImageUrl]];
    }
    if (model.taskType == 3) {
        //献贡
...
#pragma mark ---添加消耗纪念币的标签
            UILabel *label = [[UILabel alloc]init];
            label.frame = CGRectMake((SCREEN_WIDTH - 200) / 2, SCREEN_HEIGHT*0.68, 200, 50);
            label.text = [NSString stringWithFormat:@"供奉成功，消耗%ld祈福币",model.prayMoney];
            label.textColor = [UIColor whiteColor];
            label.textAlignment = 1;
            label.backgroundColor = [UIColor blackColor];
