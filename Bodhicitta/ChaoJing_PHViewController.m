...
@interface ChaoJing_PHViewController ()
...
@implementation ChaoJing_PHViewController
...
- (void)viewDidLoad {
    ...
    UILabel * title=[[UILabel alloc]initWithFrame:CGRectMake(SCREEN_WIDTH*0.25,0.01*SCREEN_HEIGHT,0.5*SCREEN_WIDTH,0.1*SCREEN_HEIGHT+10)];
    title.text=[NSString stringWithFormat:@"%@抄经排行",self.model.title];
...
{
    tableview = [[UITableView alloc] initWithFrame:CGRectMake(0,NAVBARHEIGHT*0.1+110,SCREEN_WIDTH,SCREEN_HEIGHT - NAVBARHEIGHT-5+10)];
    ...
    [tableview addHeaderWithCallback:^{
        //        [weakself getScribeRanking:_model];
    }];
    ...
    NSInteger userOrder = [[[NSUserDefaults standardUserDefaults] objectForKey:@"writeUserOrder"] integerValue];
    if (userOrder==0){
...
-(void)photoTapped{
    ...
    Wis_MyRankViewController * wis = [[Wis_MyRankViewController alloc]init];
    [wis getScribeRanking:_model];
    [self.navigationController pushViewController:wis animated:YES];
}
...
//抄经个人排行
- (void)loadPersonWrite:(CollectModel*)model
{
    AppDelegate  *appDelegate= (AppDelegate *)[UIApplication sharedApplication].delegate;
    NSNumber * miyaoNum = [appDelegate.accountBasicDict objectForKey:@"secretKey"];
    NSNumber *huaien=[appDelegate.accountBasicDict objectForKey:@"huaienID"];
    NSDictionary * dic = @{@"secretKey":miyaoNum,@"huaienID":huaien,@"taskCode":@501,@"bookID":[NSString stringWithFormat:@"%ld",(long)model.objectID],@"orderType":@"T"};
    NSDictionary * rankingDic = @{@"params":[NSString stringWithFormat:@"%@",dic]};
    
    [appDelegate.sessionManager GET:[NSString stringWithFormat:@"%@%@",__kBaseURLString,ptxGetBookOrderByUserID] parameters:rankingDic success:^(NSURLSessionDataTask *task, id responseObject) {
        id  jsonData =[NSJSONSerialization JSONObjectWithData:responseObject options:NSJSONReadingMutableContainers error:nil];
        NSLog(@"用户抄经排行=%@",jsonData);
        NSNumber *order =jsonData[@"userOrder"];
        [[NSUserDefaults standardUserDefaults]setObject:order forKey:@"writeUserOrder"];
