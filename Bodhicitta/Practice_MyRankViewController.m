//
//  Practice_MyRankViewController.m
//  Bodhicitta
//
...
@interface Practice_MyRankViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation Practice_MyRankViewController
...
-(void)getPracticeRankInfo{
    ...
    [appdelegate.sessionManager GET:[NSString stringWithFormat:@"%@%@",__kBaseURLString,GetIntegralOrder] parameters:rankingDic success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        for (int i = 0; i < rankingArray.count; i++) {
            ...
            PracticeModel * rank = [[PracticeModel alloc]init];
