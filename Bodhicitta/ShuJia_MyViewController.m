...
@interface ShuJia_MyViewController ()<UIAlertViewDelegate>
{
    //分页
    NSInteger _curPage;
    NSInteger _pageSize;
...
@property(nonatomic,strong)NSMutableArray *classicBookArray;
...
-(NSMutableArray *)dataArray
{
    if (_dataArray == nil) {
        ...
        [_dataArray addObject:self.classicBookArray];
...
- (void)viewDidLoad {
    ...
    for( int i = 0 ; i < btnTitleArray.count; i++)
    {
        ...
        btn.tag = 200 + i;
        CGFloat  marginX =  60;
        CGFloat  relativeX = SCREEN_WIDTH - 2 * marginX - 2 * SCREEN_WIDTH * 0.25;
        if (i == 0) {
            btn.selected = YES;
            selectBtn = btn;
            btn.frame = CGRectMake(marginX,(55 - 30) / 2 , SCREEN_WIDTH*0.25,30);
        }
        else
        {
            btn.frame = CGRectMake(marginX+(relativeX + SCREEN_WIDTH * 0.25), (55 - 30) / 2,SCREEN_WIDTH*0.25, 30);
        }
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (actionSheet.tag==1) {
        ...
        if (buttonIndex == 2) {
            ...
            [dujing getReadRankingWit:selectCollectModel];
        ...
        if (buttonIndex == 3) {
            ...
            [chaojing getScribeRanking:selectCollectModel];
