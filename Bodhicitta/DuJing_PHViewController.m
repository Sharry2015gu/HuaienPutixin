...
@interface DuJing_PHViewController ()
@property(nonatomic,strong)NSMutableArray * resultArray;
@property(nonatomic,strong)UILabel * nameLabel;
@property(nonatomic,strong)UILabel * levelLabel;
@end

@implementation DuJing_PHViewController
...
- (void)viewDidLoad {
    ...
    UILabel * title=[[UILabel alloc]initWithFrame:CGRectMake(SCREEN_WIDTH*0.3,0.01*SCREEN_HEIGHT,0.5*SCREEN_WIDTH,0.1*SCREEN_HEIGHT+10)];
    title.text=[NSString stringWithFormat:@"%@读经排行",self.model.title];
...
-(void)createTableView
{
    ...
    [tableview addHeaderWithCallback:^{
        //        [weakself getReadRankingWit:self.model];
    }];
    [tableview addFooterWithCallback:^{
        //        [weakself getReadRankingWit:_model];
    }];
    ...
    NSInteger userOrder = [[[NSUserDefaults standardUserDefaults] objectForKey:@"readUserOrder"] integerValue];
    if (userOrder==0){
...
#pragma mark ---读经接口----
-(void)getReadRankingWit:(CollectModel *) model
{
    NSLog(@"%ld",(long)model.collectID);
    [ProgressHUD showOnView:self.view];
    NSNumber * miyaoNum = [[NSUserDefaults standardUserDefaults] objectForKey:@"secretKey"];
    NSDictionary * dic = @{@"secretKey":miyaoNum,@"taskCode":@500,@"bookID":[NSString stringWithFormat:@"%ld",(long)model.objectID],@"orderType":@"T"};
...
//读经个人排行
- (void)loadPersonread:(CollectModel*)model
{
    AppDelegate  *appDelegate= (AppDelegate *)[UIApplication sharedApplication].delegate;
    ...
    NSDictionary * dic = @{@"secretKey":miyaoNum,@"huaienID":huaien,@"taskCode":@500,@"bookID":[NSString stringWithFormat:@"%ld",(long)model.objectID],@"orderType":@"T"};
    NSLog(@"经书id=%ld",model.objectID);
    NSDictionary * rankingDic = @{@"params":[NSString stringWithFormat:@"%@",dic]};
    ...
    [appDelegate.sessionManager GET:[NSString stringWithFormat:@"%@%@",__kBaseURLString,ptxGetBookOrderByUserID] parameters:rankingDic success:^(NSURLSessionDataTask *task, id responseObject) {
        id  jsonData =[NSJSONSerialization JSONObjectWithData:responseObject options:NSJSONReadingMutableContainers error:nil];
        NSLog(@"用户读经排行=%@",jsonData);
        NSNumber *order =jsonData[@"userOrder"];
        [[NSUserDefaults standardUserDefaults]setObject:order forKey:@"readUserOrder"];
